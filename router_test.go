...
func TestParsePattern(t *testing.T) {
	ok := reflect.DeepEqual(_parsePattern("/p/:name"), []string{"p", ":name"})
	ok = ok && reflect.DeepEqual(_parsePattern("/p/*"), []string{"p", "*"})
	ok = ok && reflect.DeepEqual(_parsePattern("/p/*name/*"), []string{"p", "*name"})
	if !ok {
